...
public sealed partial class ReportViewModel : ObservableObject
{
    ...
    private void RebootNow()
    {
        ...
        try
        {
            ...
            _services.Execution.Registry.SetValue(Core.Abstractions.RegRoot.CurrentUser,
                @"Software\Microsoft\Windows\CurrentVersion\RunOnce", "NewPcSetup", $"\"{exe}\" --resume", Core.Abstractions.RegKind.String);
            _services.Logger.Info("已写入 RunOnce，准备重启");
            Process.Start(new ProcessStartInfo("shutdown.exe", "/r /t 15 /c \"新机开荒：重启以完成设置\"") { UseShellExecute = false, CreateNoWindow = true });
            Application.Current.Shutdown();
        }
        catch (Exception ex)
        {
            _services.Logger.Error("安排重启失败", ex);
            MessageBox.Show("安排重启失败：" + ex.Message, "新机开荒", MessageBoxButton.OK, MessageBoxImage.Error);
        }
    }
