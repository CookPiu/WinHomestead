...
单窗口向导，6 屏线性推进，顶部步骤条，右上角常驻"软件入口"和"C 盘"两个独立入口。

```mermaid
flowchart LR
  S1[1 欢迎 / 电脑信息] --> S2[2 问卷]
  S2 --> S3[3 方案预览]
  S3 --> S4[4 执行进度]
  S4 --> S5[5 报告]
  S5 --> S6[6 软件入口]
...
| 屏 | 核心元素 | 交互约束 |
|---|---|---|
| 1 欢迎 | 机型/盘位/C 盘剩余卡片、"开始"按钮 | 探测未完成前按钮禁用，显示进度 |
| 2 问卷 | 5 题单页，默认值高亮，"使用默认"快捷键 | 无必填校验，全部有默认 |
| 3 方案预览 | 按模块折叠列表，每行 当前→目标 + 风险标签 + 勾选框；顶部汇总 | 取消父任务自动取消子任务 |
| 4 执行 | 进度条、当前任务、成功/失败计数、"停止" | 停止只在原子任务边界生效 |
| 5 报告 | 四类汇总、手动项卡片、导出 HTML、"立即重启/稍后" | 手动项每张卡片有跳转按钮 |
| 6 软件入口 | 分类卡片、官网、复制路径、已装标记、刷新 | 不发起任何下载 |
